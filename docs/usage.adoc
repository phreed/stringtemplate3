...
group file templates use `<...>` delimiters by default (in v2.2 `$...$` was the default delimiter). Templates created with the StringTemplate object constructor still use `$...$`.
...
Python accepts either an antlr.CharScanner class (`DefaultTemplateLexer.Lexer`, `AngleBracketTemplateLexer.Lexer` or your own implementation) or the string literals 'default' and 'angle-bracket'.
...
which gets passed to the function template as attribute body. The following code:
